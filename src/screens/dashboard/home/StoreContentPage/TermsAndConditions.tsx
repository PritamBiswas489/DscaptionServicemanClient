...
//About us content
export default function TermsAndConditionsContent({ route }: any) {
  ...
  useEffect(() => {
    const fetchHtml = async () => {
      try {
        const response = await fetch(`${getStoreAppUrl()}/terms-and-conditions`);
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: isDark ? appColors.darkTheme : appColors.white }]}>
      <Header
        title={`newDeveloper.TermsAndConditions`}
        showBackArrow={true}
      />
