...
import { loadPaymentService } from '@src/services/load.payment.service';
...
export default function Payments() {
  ...
  const {
    ...
    isNoMoreData: noMoreData } = useSelector(
      (state: RootState) => state['paymentListing'] 
    )

  const handleWithdraws = async () => {
         await loadPaymentService(limitData,offsetData,dispatch)
  ...
  const handleScrollProcessing = () => {
    ...
    dispatch(paymentListingActions.setData({ field: 'offset', data: offsetData + 1 }))
  }
